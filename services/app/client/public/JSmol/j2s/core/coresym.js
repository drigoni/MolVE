...
if (s.indexOf('.') < 0 && s.indexOf(":") < 0 && JU.PT.isDigit(s.charAt(2))) {
s += ".1";
}var info = this.getSpaceGroupJSON(this.vwr, "ITA", s, 0);
...
Clazz_overrideMethod(c$, "getCanonicalCopyTrimmed", 
function(frac, scale){
return this.unitCell.getCanonicalCopyTrimmed(frac, scale);
}, "JU.P3,~N");
...
this.unitCell.getEquivPoints(pts.get(i), flags, ops, pts, check0, n0, dup0, this.getPeriodicity());
}
...
function(nameFrom, nameTo, index1, index2){
...
var itaFrom = JU.PT.parseInt(this.getSpaceGroupInfoObj("itaNumber", nameFrom, false, false));
var itaTo = (nameTo == null ? -1 : nameTo.length == 0 ? 0 : JU.PT.parseInt(this.getSpaceGroupInfoObj("itaNumber", nameTo, false, false)));
...
return resource;
}var typeName = JS.SpaceGroup.getSpecialGroupName(type);
var nGroups = JS.SpaceGroup.getMax(type);
var data = null;
switch (type) {
case 300:
if (JS.Symmetry.friezeSubData == null) JS.Symmetry.friezeSubData =  new Array(nGroups);
data = JS.Symmetry.friezeSubData;
break;
case 400:
if (JS.Symmetry.layerSubData == null) JS.Symmetry.layerSubData =  new Array(nGroups);
data = JS.Symmetry.layerSubData;
break;
case 500:
if (JS.Symmetry.rodSubData == null) JS.Symmetry.rodSubData =  new Array(nGroups);
data = JS.Symmetry.rodSubData;
break;
case 600:
if (JS.Symmetry.friezeSubData == null) JS.Symmetry.friezeSubData =  new Array(nGroups);
data = JS.Symmetry.friezeSubData;
...
var resource = data[itno - 1];
if (resource == null) data[itno - 1] = resource = JS.Symmetry.getResource(this.vwr, "sg/json/sub_" + typeName + "_" + itno + ".json");
return resource;
}, "~N,~N");
c$.getITJSONResource = Clazz_defineMethod(c$, "getITJSONResource", 
function(vwr, type, itno, name){
if (type == 0) {
...
return JS.Symmetry.getSpecialSettingJSON(data, name);
}, "JV.Viewer,~N,~N,~S");
...
;//5.0.1-v4 Fri Oct 11 09:17:57 CDT 2024
Clazz_declarePackage("JS");
...
this.allOperations = JS.SymmetryOperation.getAdditionalOperations(this.finalOperations, this.periodicity);
...
var sg = lst.get(i);
if (clegId != null && clegId.equals(sg)) return JS.SpaceGroup.SG[sg.index];
...
var s = (specialType > 0 ? name.substring(2) : name);
var ptCleg = s.indexOf(":");
var ptTrm = s.indexOf(",");
var ptIndex = s.indexOf(".");
var pt = (ptCleg > 0 && ptTrm > ptCleg ? ptCleg : ptIndex);
var itno = JS.SpaceGroup.getITNo(s, pt);
var itindex = (pt > 0 && pt == ptCleg || itno == -2147483648 ? 0 : pt > 0 ? JS.SpaceGroup.getITNo(s.substring(pt + 1), 0) : 1);
if (specialType > 0) {
return JS.Symmetry.getSGFactory().getSpecialGroup(null, vwr, name, itno, itindex, pt > 0 && pt == ptCleg, specialType);
...
for (var i = n; --i >= 0; ) if (!JU.PT.isDigit(name.charAt(i))) return -1;
...
c$.hmEquals = Clazz_defineMethod(c$, "hmEquals", 
function(hm, name, type){
...
;//5.0.1-v4 Fri Oct 11 09:41:37 CDT 2024
Clazz_declarePackage("JS");
...
return this.sg;
}this.sg = JS.SpaceGroup.transformSpaceGroup(this.sg == null ? 0 : this.sg.groupType, null, this.sg, genPos, (hasTransform ? transform : null), (hasTransform ?  new JU.M4() : null));
if (this.sg == null) return null;
name = "";
...
System.out.println("SGF read " + list.length + " records for " + fileName);
return list;
}, "JV.Viewer,~A,~S");
...
function(xyzList, lst, mapCoincident, n0, isym, vTemp, periodicity){
var t0 =  new JU.V3();
...
}, "JU.SB,JU.Lst,java.util.Map,~N,~N,JU.V3,~N");
...
;//5.0.1-v4 Thu Oct 10 16:16:22 CDT 2024
Clazz_declarePackage("JS");
...
this.additionalOperations = JS.SymmetryOperation.getAdditionalOperations(this.symmetryOperations, 0x7);
...
;//5.0.1-v4 Fri Oct 11 09:37:16 CDT 2024
Clazz_declarePackage("JS");
...
;//5.0.1-v4 Thu Oct 10 12:55:18 CDT 2024
Clazz_declarePackage("JS");
...
var key = sg.specialPrefix + cleg;
var helper = JS.WyckoffFinder.helpers.get(key);
...
JS.WyckoffFinder.helpers.put(key, JS.WyckoffFinder.nullHelper);
} else {
JS.WyckoffFinder.helpers.put(key, helper);
}return helper;
...
var key = JS.SpaceGroup.getGroupTypePrefix(groupType) + clegId;
var sgname = clegId;
var pt = sgname.indexOf(":");
...
var err = JS.CLEG.assignSpaceGroup(data,  new JS.CLEG.AssignedSGParams(vwr));
if (err.indexOf("!") > 0) {
...
if ((isSubgroupCalc || JS.CLEG.isTransformOnly(token)) != asgParams.mkWasNode) {
...
var paramsInit = (asgParams.mkCalcOnly ?  new JS.CLEG.AssignedSGParams(vwr) :  new JS.CLEG.AssignedSGParams(vwr, null, null, asgParams.mkParamsOrUC, -1, true, asgParams.mkSb, false));
...
this.myTrm = (name.endsWith(".1") ? "a,b,c" : data.sym.getITASettingValue(null, this.specialPrefix + name, "trm"));
if (this.myTrm == null) {
...
var o = data.sym.getSubgroupJSON((isSub ? prev.name : this.name), (isSub ? this.name : prev.name), 0, 1);
trCalc = o;
