...
this.vwr.showString("could not move atom!", false);
...
var plane1 = null;
var line1 = null;
for (var i = ops.length + (isPlanar ? 1 : 0); --i >= 0; ) {
var line2 = null;
var c = (i == ops.length ? J.modelkit.ModelKit.plane001 : sym.getSymmetryInfoAtom(this.vwr.ms, iatom, null, ops[i], null, a, null, "invariant", 1275068418, 0, -1, 0, null));
if ((typeof(c)=='string')) {
continue;
...
pts[0] = allPts[i];
n += this.addAtoms(i == 0 ? type : elements.substring(ept, ept + 2).trim(), pts, bsAtoms, packing, opsCtr, cmd);
}
return n;
...
var per = sym.getPeriodicity();
var shiftA = (per == 0x4 && nDim == 3);
var shiftB = (shiftA || per == 0x1 && nDim == 2);
var shiftC = (per == 0x1 && nDim == 2 || per == 0x3 && nDim == 3);
var pts = axisPoints;
var opts = null;
...
s += "\ndraw ID " + JU.PT.esc(id + "_axis_" + JV.JC.axisLabels[a]) + " " + swidth + " line " + (opts == null || i == 0 && !shiftA || i == 1 && !shiftB ? origin : opts[i]) + " " + pts[i] + " color " + colors[i];
}
return s;
}, "~S,~S");
